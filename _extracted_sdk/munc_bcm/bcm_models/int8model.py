...
FACTORY_NAME = 'munc_int8'


class Int8Attributes:
    def __init__(self):
        pass


class PytorchInt8MMA:
    """An int8 model of the mma
    ...
    def __init__(self, weights, biases, mma_attr=None, pFSR=2.0, iFSR=2.0, name=None, seed=None):
        ...
        if pFSR != iFSR:
            raise ValueError(f"pFSR and iFSR are not equal in an int8 node. pFSR={pFSR}, iFSR={iFSR}.")

        self.iFSR = iFSR
        self.pFSR = pFSR
        self.mma_attr = mma_attr or Int8Attributes()
        self.weights = (pFSR / iFSR) * 128 * weights
        self.biases = (pFSR / iFSR) * 128 * biases.sum(1) * 255
