...
**各成分の意味**(出典: [05_all_digital_ppa.md](05_all_digital_ppa.md) §2.3, §5.1): MAC unit/Non-MAC unitは「MAC/non-MACサイクル数×1サイクルあたりのpJ」で課金される演算ユニット本体のコスト。DMEM(Data Memory、1/2/3の3プール、既定サイズ2048/24576/32768B)は各v-MP/クラスタに近いローカルなデータスクラッチパッド、IMEM(Instruction Memory)は実行すべき命令ストリームを保持するメモリで、いずれも「アクセス回数×1回あたりのpJ」で課金される。OCRAM(既定32MB/4MB)はDMEMより大きく広く共有されるオンチップSRAM(`M`=MP単位アクセス・`C`=Cluster単位アクセスの2種類のポートで別課金)。DDRはチップ外部のDRAM(バイト数課金)。Bus/NoCはメモリ階層間・クラスタ間の配線網(DDR外部インターフェース用バス・MP↔Cluster間バス・クラスタ間ネットワークの総称)。
...
**「計算」と「データの保存・移動」で二分すると、後者が過半を占める。** 6成分は本質的に2グループに分かれる: 演算ユニット本体(MAC unit+Non-MAC unit、実際にデータへ処理を行う回路)と、データ・命令の保存・転送(DMEM+IMEM+OCRAM+DDR+Bus/NoC、演算そのものは行わない回路)。前者は12.7%+28.3%=**41.0%**、後者は34.7%+18.4%+5.9%+0.0%=**59.0%**であり、**「データの保存・移動」側が「計算」側を上回る**。ここで注意が必要なのは、Non-MAC unitは名前に反して演算ユニット(計算側)に分類される点である——elementwise演算やSoftmaxの正規化も実際にデータへ処理を行う仕事であり、単なる保存・転送ではない。「データの保存・移動」に属するのはDMEM/IMEM/OCRAM/DDR/Bus・NoCの5成分のみである。この59.0% vs 41.0%という二分は、アナログ側(§3-10、SRAM-boundなBEVFormerではデータ移動系51.1% vs ACE 48.9%)と同じ非対称性がデジタル側でもさらに強く表れていることを示す。

**補足: MAC unitとNon-MAC unitの1サイクルあたりコストには約9.2倍の非対称性がある。** §2.2のサイクル内訳(MAC 71.1%・non-MAC 17.2%)と本節の電力寄与(MAC 12.7%・non-MAC 28.3%)から、1サイクルあたりのエネルギーを逆算すると、MAC unitが約730 pJ/cycle、Non-MAC unitが約6,738 pJ/cycleとなる(`Power@eff.fps`基準の寄与額 ÷ 総実行時間(=1/eff. fps) ÷ 各サイクル数から算出)。**Non-MAC unitはサイクル数の絶対量ではMAC unitの4分の1程度しかないにもかかわらず、1サイクルあたりのコストがMAC unitの約9.2倍あるため、電力全体ではMAC unitより大きな比率を占める。** この非対称性の回路的な理由(なぜNon-MAC単位のpJ/cycle定数がMAC単位よりはるかに大きいのか)は、ブラックボックス測定では特定できず、`vnnmap`の逆アセンブルが必要な領域である**[推測、未検証]**。
