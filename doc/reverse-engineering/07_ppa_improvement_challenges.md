...
**レイテンシと電力で「ボトルネック」の意味が異なる点に注意。** レイテンシは`max(ACEクリティカルパス, SRAM時間)`という構造(§3-1)のため、支配していない側(BEVFormerではACE)をいくら改善してもレイテンシは変わらない。一方、電力は`ACE + SRAM + Accessor + Control + NOC`という単純な**加算**構造(§3-10)のため、どの項を減らしても電力は必ず減る。BEVFormerの電力内訳(ACE 48.9% vs データ移動系51.1%、§6.2)は僅差であり、「データ移動が電力のボトルネック」という記述は現状の内訳比較にすぎず、**ACE側(クロスバー充填率向上、§4.3)による電力改善が無効という意味ではない**。データ移動削減を優先する理由は、それがレイテンシ側の律速要因(SRAM時間)とも一致するため、レイテンシ・電力の両方を同時に改善できる点にある。ACE充填率向上は電力には独立に効くが、SRAM時間がmax()の支配項である限りレイテンシには効かない。
...
**含意**: BEVFormerとYOLOPXは、レイテンシ・電力の両軸で「データ移動系(SRAM/Accessor/Interconnect/DMEM/IMEM)が支配的か、演算系(ACE/MAC)が支配的か」という同一の非対称性を、アナログ・デジタルの両方の実行系統で共有している。BEVFormer(SRAM-bound)はアナログでもデジタルでもデータ移動・低密度演算が僅差ながら支配的コストであり、レイテンシ・電力の両方に効く最優先の改善指針は両系統で「データ移動量・低密度演算の削減」に収束する(§6.3の注記の通り、電力は加算構造のためACE側の充填率改善も独立に有効だが、レイテンシには効かない)。YOLOPX(ACE-bound)はアナログ側がACEそのものに支配され、デジタル側は総電力に対してほぼ無視できる規模にとどまる。**この対称性は、律速要因の判定(§4.1)が電力側だけでなくデジタル側の改善対象判定にも使える、という§3-10の結論を裏付けている。**
