...
- [3. 主要課題](#3-主要課題)
- [4. SRAMアクセス削減に適したモデル構造](#4-sramアクセス削減に適したモデル構造)
...
2. レイテンシ改善(SRAMアクセス削減)に適したモデル構造とは何か(§4)。
...
## 4. SRAMアクセス削減に適したモデル構造

質問「レイテンシ改善にはSRAMアクセスを減らすことが必要だと思うが、適したモデル構造は何か」への回答。

### 4.1 まず律速判定を行う(構造変更より先にすべきこと)

SRAM削減が有効なのは、そのモデルが**SRAM-bound**である場合に限られる(§3-1)。ACE-boundなモデル(YOLOPX)に対してSRAM削減を目的にモデル構造を変えても、レイテンシはACEクリティカルパスで頭打ちになったままで改善しない。既存の`_ppa_*.tar.gz`から`tools/perf_breakdown/perf_breakdown.sh`で約9秒で判定できる(出典: [02_ppa_estimation.md](02_ppa_estimation.md) §3.9(4))ため、モデル構造の変更に着手する前に必ず行うべき最初のステップである。

### 4.2 SRAM負荷の主因は「並列化のための重み複製」

§3-2で示した通り、SRAMトラフィックの増加はACE数を増やした際の重み複製に強く結びついている(YOLOPX 48→72で重み+18.0%、SRAM読+5.3%/書+7.3%)。したがって「SRAM削減に適したモデル構造」は、言い換えれば**「同じACE数でも重み複製が少なくて済む構造、またはSRAM往復あたりの計算密度が高い構造」**である。

### 4.3 具体的な指針(既存実測からの含意)
...
1. **KVキャッシュ増大によるSRAM/DDRトラフィック増加。** BEVFormerが6カメラ入力でSRAM-bound化した構造(§3-1, §4.3)と同型のリスクが、シーケンス長の増大でも起こりうる**[推測]**。層数・シーケンス長が増えるほど、デジタル側のメモリトラフィックがボトルネックになる可能性が高い。
...
- Attention部分をoff-chip一括処理からアナログMMAへの変換に載せ替えた場合のSRAMトラフィック変化(§4.3で[推測]とした点)の実測。
...
| 重み複製がarea↔SRAM↔powerを結合(3-2) | 重み再利用度の高い演算を優先(4.3) | `num_aces` |
...
| コンパイラ非決定性・既知バグ(3-8) | 重み容量の事前計算でコンパイル試行を減らす(4.3) | — |
| A_max未確定(3-9) | モデル軽量化(約24%短縮目標)かA_max見直しの二択 | — |
| depthwise Convは強制デジタル化 | on-chip密度を優先する場合は比率を絞る(4.3) | `MarkDepthwiseConvsAsDigital` |
| 多カメラ・多視点入力はSRAM負荷増(4.3) | 特徴マップ解像度・カメラ数に注意 | — |
| Attentionはoff-chip一括処理になりやすい(4.3) | grouped Conv変換の効果は未実測 | `AttentionDetr`等のRewriteRule |
